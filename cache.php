...
class cache {
	...
	private static function init(){
		...
		global $CONFIG;
		if ($CONFIG['MEMCACHE']==1 && class_exists('Memcache')){//Проверка доступности и включённости Memcache
			if ($CONFIG['memcache_servers']!==null){
				cache::$flag=($CONFIG['memcache_compress']===true)?MEMCACHE_COMPRESSED:0;
				cache::$memcache = new Memcache;
				cache::$connected = false;
				foreach ($CONFIG['memcache_servers'] as $connection) {
					$host=$connection[0];
	 ...
	 * @param integer $expire - время жизни запроса в кеше в секундах (если числовое значение), или timestamp смерти запроса в кеше (если UNIX_TIMESTAMP). По умолчанию равно параметру $CONFIG['memcache_expriration'].
	...
	public static function select ($query,$ignore_cache=FALSE,$tags=array(),$expire=0,$error_behavior=2){
		...
		if (!$ignore_cache && $CONFIG['MEMCACHE']==1 && cache::init()){
